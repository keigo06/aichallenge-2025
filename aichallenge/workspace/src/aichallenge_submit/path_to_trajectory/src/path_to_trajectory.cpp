...
PathToTrajectory::PathToTrajectory() : Node("path_to_trajectory_node") {
  ...
  objects_sub_ = this->create_subscription<MarkerArray>(
      "/aichallenge/objects_marker", rv_qos,
      [this](const MarkerArray::SharedPtr msg) {
        objects_ = msg;

        // 障害物の総数を出力
        RCLCPP_INFO(
          this->get_logger(),
          "Received objects (total: %zu)",
          objects_->markers.size());
        ...
        for (size_t i = 0; i < objects_->markers.size(); ++i) {
          ...
          RCLCPP_DEBUG(
            this->get_logger(),
            "Object[%zu]: position=(%.2f, %.2f, %.2f), diameter=%.2f, frame_id=%s",
            i,
